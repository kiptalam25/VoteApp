...
//    }
    ...
    public MutableLiveData<Boolean> addChatsInDatabase(String receiverId, String senderId, String message, String timestamp) {
        ...
        instance.getReference("Chats").push().setValue(hashMap).addOnCompleteListener(task -> successAddChatsDb.setValue(true)).addOnFailureListener(e -> successAddChatsDb.setValue(false));
    ...
    public MutableLiveData<Boolean> addBioInDatabase(String bioUpdated, Object bio) {
        ...
        reference.updateChildren(map).addOnCompleteListener(task -> successAddBio.setValue(true)).addOnFailureListener(e -> successAddBio.setValue(false));
...
//        databaseReference=db.getReference(Users.class.getSimpleName());/
    ...
    public MutableLiveData<Boolean> addUserInDatabase(Users user) {
        ...
        instance.getReference(Users.class.getSimpleName())
                .child(user.getId())
                .setValue(user)
                .addOnCompleteListener(task ->
                        successAddUserDb.setValue(true)).addOnFailureListener(e -> successAddUserDb.setValue(false));

        return successAddUserDb;
    }


    public LiveData<Boolean> placeOrder(Orders order) {
        final MutableLiveData<Boolean> successPlaceOrder = new MutableLiveData<>();


        DatabaseReference orderRef=instance.getReference("Orders").child(firebaseUser.getUid());
        orderRef.push().setValue(order).addOnCompleteListener(task -> successPlaceOrder.setValue(true)).addOnFailureListener(e -> successPlaceOrder.setValue(false));

        //creating chatList in database for better performance in chatListFragment .

//        DatabaseReference chatRef = instance.getReference("OrderList")
//                .child(buyer)
//                .child(seller);
